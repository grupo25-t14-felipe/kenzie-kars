...
interface iUpdateAnnouncement {
  ...
  setAnnouncemets: Dispatch<SetStateAction<Omit<iAnnouncement, "user"> | any>>;
...
const UpdateAnnouncement = ({
  ...
  const updateAnnouncement = async ({ mileage, image, ...data }: iUpdateAnnouncementRequest) => {
    ...
    const res = await api.get(`/users/${jwt_decode<any>(token).sub}`);
    setAnnouncemets(res.data.announcement);
    
    if (image) {
      image.forEach(async ({ id, link }) => {
        id?.startsWith("id-")
          ? await api
              .post(`/image/announcement/${announcement?.id}`, { link: link })
              .then(async (res) => {})
              .catch((err) => {
                console.error(err);
              })
    ...
    <div className="w-screen h-screen p-2 bg-grey-0/25 flex content-center fixed z-[21] top-0 cursor-default">
      <div className="min-w-[300px] max-w-[400px] p-2 m-auto bg-whiteFixed rounded-[4px]">
        ...
        <form
          ...
          <div className="flex flex-row gap-2.5">
            <div className="w-1/2">
              <label htmlFor="price_table_fipe" className="input-label">
                Preço tablela FIPE
              </label>
              <input
                type="text"
          ...
          <label htmlFor="published" className="input-label">
            Publicado
          </label>
          ...
          <label htmlFor="cover-image" className="input-label">
            Imagem da capa
          </label>
          <input
            type="text"
